...
import modeling.DA_TransUNet_config as configs
from .DA_TransUNet_Block import ResNetV2
from .DA_TransUNet_Block import DANetHead
...
class DA_Transformer(nn.Module):
    def __init__(self, config=configs.get_r50_b16_config(), img_size=256, num_classes=9, zero_head=False, vis=False):
    ...
    def load_from(self, weights=np.load("/home/wpf/SSD/XYQ/medical_image_segmentation/modeling/R50+ViT-B_16.npz")):
        with torch.no_grad():
...
if __name__ == '__main__':
    ...
    model = DA_Transformer().to("cuda:0")
    model.load_from()
    y = model(x)
    print(y.shape)
