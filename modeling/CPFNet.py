...
def resnet34(pretrained=False, **kwargs):
    ...
    if pretrained:
        pretrained_dict = model_zoo.load_url(model_urls['resnet34'],
                                             model_dir='modeling/resnet34')  # Modify 'model_dir' according to your own path
...
class GPG_2(nn.Module):
    def __init__(self, in_channels, width=512, up_kwargs=None, norm_layer=nn.BatchNorm2d):
        ...
        self.dilation1 = nn.Sequential(
            SeparableConv2d(4 * width, width, kernel_size=3, padding=1, dilation=1, bias=False),  # (256,64,3,1,1)
            nn.BatchNorm2d(width),
            nn.ReLU(inplace=True))

        self.dilation2 = nn.Sequential(
            SeparableConv2d(4 * width, width, kernel_size=3, padding=2, dilation=2, bias=False),  # (256,64,3,2,2)
            nn.BatchNorm2d(width),
            nn.ReLU(inplace=True))

        self.dilation3 = nn.Sequential(
            SeparableConv2d(4 * width, width, kernel_size=3, padding=4, dilation=4, bias=False),  # (256,64,3,4,4)
            nn.BatchNorm2d(width),
            nn.ReLU(inplace=True))

        self.dilation4 = nn.Sequential(
            SeparableConv2d(4 * width, width, kernel_size=3, padding=8, dilation=8, bias=False),  # (256,64,3,8,8)
            nn.BatchNorm2d(width),
            nn.ReLU(inplace=True))
    ...
    def forward(self, *inputs):
        ...
        feats = [self.conv5(inputs[-1]), self.conv4(inputs[-2]), self.conv3(inputs[-3]),
                 self.conv2(inputs[-4])]  # [c5,c4,c3,c2]
...
class GPG_3(nn.Module):
    def __init__(self, in_channels, width=512, up_kwargs=None, norm_layer=nn.BatchNorm2d):
        ...
        # [4,512*3,32,32]
        self.dilation1 = nn.Sequential(
            SeparableConv2d(3 * width, width, kernel_size=3, padding=1, dilation=1, bias=False),  # (512*3,512,3,1,1)
            nn.BatchNorm2d(width),
            nn.ReLU(inplace=True))

        self.dilation2 = nn.Sequential(
            SeparableConv2d(3 * width, width, kernel_size=3, padding=2, dilation=2, bias=False),  # (512*3,512,3,2,2)
            nn.BatchNorm2d(width),
            nn.ReLU(inplace=True))

        self.dilation3 = nn.Sequential(
            SeparableConv2d(3 * width, width, kernel_size=3, padding=4, dilation=4, bias=False),  # (512*3,512,3,4,4)
            nn.BatchNorm2d(width),
            nn.ReLU(inplace=True))
...
class GPG_4(nn.Module):
    def __init__(self, in_channels, width=512, up_kwargs=None, norm_layer=nn.BatchNorm2d):
        ...
        self.dilation1 = nn.Sequential(
            SeparableConv2d(2 * width, width, kernel_size=3, padding=1, dilation=1, bias=False),  # (1024,512,3,1,1)
            nn.BatchNorm2d(width),
            nn.ReLU(inplace=True))  #

        self.dilation2 = nn.Sequential(
            SeparableConv2d(2 * width, width, kernel_size=3, padding=2, dilation=2, bias=False),  # (1024,512,3,2,2)
            nn.BatchNorm2d(width),
            nn.ReLU(inplace=True))  #
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
...
class SeparableConv2d(nn.Module):
    ...
    def __init__(self, inplanes, planes, kernel_size=3, stride=1, padding=1, dilation=1, bias=False,
        ...
        # input [4,2048,64,64]
        self.conv1 = nn.Conv2d(inplanes, inplanes, kernel_size, stride, padding, dilation, groups=inplanes,
                               bias=bias)  # out [4,2048,64,64]
...
class SAPblock(nn.Module):
    ...
    def forward(self, x):  # x [4,512,8,8]
        ...
        ax = self.relu(self.gamma * (att_1_2 * fusion_1_2 + att_3 * branches_3) + (
                    1 - self.gamma) * x)  # gamma*[4,512,8,8] + (1-gamma)*[4,512,8,8] = [4,512,8,8]
...
class ConvBnRelu(nn.Module):  # (in,in,3,1,1)#(in,in//2,1,1,0)
    def __init__(self, in_planes, out_planes, ksize, stride, pad, dilation=1, groups=1, has_bn=True,
...
class CPFNet(nn.Module):
    def __init__(self, num_classes=1, ccm=True, norm_layer=nn.BatchNorm2d, is_training=True, expansion=2,
        ...
        self.main_head = BaseNetHead(spatial_ch[0], num_classes, 2, is_aux=False, norm_layer=norm_layer)
